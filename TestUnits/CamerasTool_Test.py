...
def guiTest():
    ...
    camerasToolButtonLocation = tool.locateCenterOnScreen('Horizon Tool' ,testFolder + 'cameras_icon.png')
    ...
    if ((screenSize[0] * 0.03) <= camerasToolButtonLocation[0] <= (screenSize[0]*0.035)):
        p.click(camerasToolButtonLocation[0], camerasToolButtonLocation[1], clicks=1, interval=0, button='left')
    else:
        print('Cannot click on the cameras tool, maybe it has already been clicked on?')
    ...
    p.click((screenSize[0] * 0.4), (screenSize[1] * 0.3))
